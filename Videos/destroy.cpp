...
class QuadTreeNew
{
    ...
    static constexpr int split_factor = 20;
public:
    QuadTreeNew(const olc::vi2d& upper_left, Width width, Height height, Level level):
        rect{ upper_left, width, height }, level{ level } { }
...
private:
    ...
    void split()
    {
        ...
        trees[0] = std::make_unique<QuadTreeNew>(olc::vi2d{ x + sub_width, y },              Width(sub_width), Height(sub_height), Level(next_level));
        trees[1] = std::make_unique<QuadTreeNew>(olc::vi2d{ x,             y },              Width(sub_width), Height(sub_height), Level(next_level));
        trees[2] = std::make_unique<QuadTreeNew>(olc::vi2d{ x,             y + sub_height }, Width(sub_width), Height(sub_height), Level(next_level));
        trees[3] = std::make_unique<QuadTreeNew>(olc::vi2d{ x + sub_width, y + sub_height }, Width(sub_width), Height(sub_height), Level(next_level));
    }
    ...
    std::array<std::unique_ptr<QuadTreeNew>, max_depth> trees;
    std::vector<PhysicsPixel*> objects;
    BoundingBox rect;
    const Level level;
};
